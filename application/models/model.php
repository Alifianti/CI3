...
class Model extends CI_Model {
	...
	public function GetArtikel(){
		...
		$this->db->select('id,tempat,tgl,img,judul,caption');
		$this->db->from('iwwm');
		$data=$this->db->get();
	...
	public function GetPreview($id=''){
		$isi = $this->db->query('SELECT id,tempat,tgl,img,judul,caption FROM iwwm where id = '.$id);
	...
	public function getedit($id=''){
		$data = $this->db->query('SELECT id,tempat,tgl,img,judul,caption FROM iwwm where id  = '.$id);
		return $data->result_array();
	}
